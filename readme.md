...
带 🔒 的 skill 为「manual-only」
模型不能自动调用，但用户可以显式调用, skill name&description不占上下文, 适合上下文洁癖患者。
...
`langchain-dev-guide` 来自 [agentseek](https://github.com/ob-labs/agentseek)；`grill-request`、`prototype` 受 [mattpocock/skills](https://github.com/mattpocock/skills) 启发。
